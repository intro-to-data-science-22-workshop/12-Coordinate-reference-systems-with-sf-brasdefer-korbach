...
**After listening to the presentation, talk to your neighbor, briefly define the purpose of a CRS and discuss why it is important when dealing with spatial data.**

Here you have space for notes:
...
The `german_districts` data set is saved as an `sf` object which at the same time is a data frame. That's great! As a consequence, we can effortlessly use tidyverse methods to wrangle our data.
...
Similar to the calculation of area, there are other functions in the `sf` package that you can use to do computations with simple features such as `st_length()`or `st_area()`.
...
An easy way to graph maps is the `plot()` command that create a set of default maps corresponding to the attributes of our `sf` object. Let's plot the first three attributes
